...
export function registerVerificationSkills(add,runtime) {
  ...
  if (!queries) throw new TypeError('registerVerificationSkills requires runtime.queries');
  if (!commands) throw new TypeError('registerVerificationSkills requires runtime.commands');
  add('validateWorld', meta('执行确定性的几何、物理与关系校验。', ['world.read', 'physics.read']), () => queries.validateWorld());
  add('repairWorld', { ...meta('修复硬错误，并拒绝使结果更差的修复。', ['world.write', 'physics.read'], [], { report: { type: 'object' }, maxRepairs: { type: 'integer' } }), mutates: true }, (a) => commands.repair(a.report || queries.validateWorld(), { maxRepairs: a.maxRepairs ?? 20 }));

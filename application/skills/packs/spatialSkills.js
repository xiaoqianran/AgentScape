...
import { listObservedEntities, resolveObservedEntity } from '../../../modules/world/runtime/ObservedEntity.js';
...
export function registerSpatialSkills(add,runtime) {
  ...
  add('overlaps', meta('检查对象的空间包围盒是否与其他对象重叠；这不是 Physics contact/collision 查询。', ['spatial.read'], ['id'], { id: string, ignore: { type: 'array', items: string }, margin: number }), (a) => runtime.spatial.overlappingIds(a.id, { ignore: a.ignore ?? [], margin: a.margin ?? 0.01 }));
  ...
  add('getLocomotionStatus', meta('读取 Agent Body 当前或最近一次 locomotion 状态。', ['world.read', 'physics.read'], ['id'], { id:string }), (a) => runtime.locomotionStatus(a.id));
  add('findInteractionPose', meta('只读诊断/预览：按 Runtime 固定 1.5m 交互距离，为 Agent 与目标寻找满足当前 navigation backend 可达和 physics scene-query 视线的交互位；可选 action/partName 时排除 Agent 阻挡 articulation sweep 的位姿。若目标是实际走过去并 open/close，应直接调用 approachAndInteract，不要手工拆链。', ['spatial.read', 'physics.read'], ['actorId','targetId'], { actorId:string, targetId:string, action:{type:'string',enum:['open','close']}, partName:string }), (a) => runtime.findInteractionPose(a.actorId, a.targetId, { action:a.action, partName:a.partName }));

...
for(const retired of [
  ['application','generation','LegacyAuthoringShell.js'],
  ['modules','generation','artifacts'],
  ['modules','asset','gateway','HttpAssetGenerator.js'],
  ['api','capabilities','asset-generate.js'],
  ['dev','scripts','repos.sh'],
  ['sdk','python','agentscape','pipeline.py'],
  ['apps','studio','ui','build','BuildWorkbench.js'],
  ['apps','studio','ui','inspect','ObjectInspector.js'],
  ['apps','studio','demos','generated-placement','index.js'],
  ['modules','agent','prompt','basePolicy.js'],
  ['modules','agent','prompt','mutationPolicy.js'],
  ['modules','agent','prompt','recoveryPolicy.js'],
  ['modules','agent','prompt','worldPolicy.js'],
  ['modules','agent','prompt','embodiedPolicy.js'],
  ['sdk','python','agentscape','providers']
]) if(exists(...retired)) failures.push(`Retired surface must not return: ${retired.join('/')}`);

const generationFiles=walk(path.join(root,'modules','generation')).filter((file)=>file.endsWith('.js'));
for(const file of generationFiles){
  ...
  for(const match of source.matchAll(/(?:from\s+|import\s*\()\s*['"]([^'"]+)['"]/g)){
    ...
    if(target.startsWith('apps/studio/')) failures.push(`Generation must not depend on Studio: ${rel(file)} -> ${target}`);
  }
}

const providerRegistry=read('modules','generation','providers','ProviderRegistry.js');
...
const toolAgent=read('modules','agent','ToolCallingAgent.js');
...
const coreSkills=read('application','skills','registerCoreSkills.js');
...
if(!exists('modules','agent','prompt','index.js')) failures.push('Structured Agent prompt policies are missing');
if(!exists('application','skills','packs')) failures.push('Domain skill packs are missing');

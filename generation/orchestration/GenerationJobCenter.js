...
<section class="generation-console" aria-label="生成任务中心">
  <div class="generation-heading"><div><div class="eyebrow">创建</div><h2>创建资产</h2><p>生成内容、完成验证，然后将资产加入当前世界。</p></div><span id="generation-connector-badge" class="generation-badge">本地可选</span></div>
  <div id="generation-state" class="generation-state" data-state="ready"><strong id="generation-state-label">检查生成能力</strong><span id="generation-state-detail">服务器能力可直接使用；本地适配器仅用于可选扩展。</span></div>
  <div class="generation-scroll">
    <details class="generation-section"><summary>本地适配器（可选）</summary><div class="generation-section-body">
      ...
      <small>仅本地资源或本地凭据能力需要连接；普通服务器能力不依赖本地适配器。</small>
      <small id="generation-pairing-hint">本地适配器只承载本机资源或本机凭据；普通生成能力不依赖它。</small>
    </div></details>
    <details class="generation-section" open><summary>创建设置</summary><div class="generation-section-body">
      ...
      <label class="generation-confirm"><input id="generation-cost-confirm" type="checkbox" />我确认生成可能使用外部计算资源并产生费用。</label>
...
export function capabilityHint(capability) {
  ...
  const connection=capability.connectionRequired?'需要本地适配器':'直接能力';
...
this.buttons.pair.addEventListener('click',()=>this.pair());this.buttons.revoke.addEventListener('click',()=>this.revoke());this.buttons.submit.addEventListener('click',()=>this.submit());this.buttons.refresh.addEventListener('click',()=>this.refresh({remote:true}));this.buttons.jobRefresh.addEventListener('click',()=>this.refreshSelected());this.buttons.cancel.addEventListener('click',()=>this.cancelSelected());this.buttons.import.addEventListener('click',()=>this.importSelected());this.buttons.compile.addEventListener('click',()=>this.compileSelected());this.buttons.spawn.addEventListener('click',()=>this.spawnSelected());this.provider.addEventListener('change',()=>this.renderCapabilities());this.operation.addEventListener('change',()=>this.renderCapabilityHint());this.costConfirm.addEventListener('change',()=>this.renderCapabilityHint());
    ...
    this.refreshCapabilities();this.renderConnectorState(this.world.generation.connectorStatus());await this.refresh({remote:this.world.generation.connectorStatus().status==='paired'});
  ...
  hasDirectGenerationCapability(){return this.capabilities.some((capability)=>capability.status==='available'&&!capability.connectionRequired&&capability.input?.types?.includes('text')&&capability.output?.roles?.includes('asset'));}
  renderConnectorState(state={}){
    const connectorState=this.world.generation.connectorStatus();
    const paired=connectorState.status==='paired'||Boolean(state.connector&&state.status==='generation-ready');
    this.badge.textContent=paired?'本地已连接':'本地可选';
    this.badge.classList.toggle('live',paired);
    if(paired){
      const connector=state.connector||connectorState.connector;
      this.setState('ready','生成能力可用',`本地适配器 ${text(connector?.id,'已连接')} · 服务器能力仍可独立使用`);
      this.pairingHint.textContent='本地适配器会话已建立；只影响本地扩展能力。';
    } else if(state.reason==='APPROVAL_REQUIRED'){
      this.pairingId=state.pairingId||this.pairingId;
      this.setState('approval-required','等待本地适配器批准','本地扩展能力等待批准；服务器能力不受影响。');
      this.pairingHint.textContent=`配对 ID：${text(this.pairingId)} · 不包含提供方凭据。`;
    } else if(this.hasDirectGenerationCapability()){
      this.setState('ready','生成能力可用','服务器适配器可直接生成；本地适配器未连接。');
      this.pairingHint.textContent='本地适配器是可选扩展，只用于本地资源或本地凭据。';
    } else {
      this.setState('connection-required','暂无可用生成能力','可配置服务器生成适配器，或连接可选的本地适配器。');
      this.pairingHint.textContent='本地适配器不是必需组件；只有本地能力才需要它。';
    }
    this.buttons.revoke.disabled=!paired;
    this.renderCapabilityHint?.();
  }

  async pair(){try{this.setState('working','连接中','正在连接可选的本地适配器…');const result=await this.world.generation.pairConnector({pairingId:this.pairingId});this.renderConnectorState(result);if(result.status==='generation-ready'){this.pairingId=null;this.world.generationState=clone(result);this.refreshCapabilities();await this.refresh({remote:true});}}catch(error){this.setState('error','本地适配器连接失败',error.message);this.log(`本地适配器错误：${error.message}`,'error');}}
  async revoke(){try{const result=await this.world.generation.revokeConnector();this.world.generationState=clone(result);this.renderConnectorState(result);this.log('本地适配器会话已撤销','result');}catch(error){this.setState('error','撤销失败',error.message);this.log(`撤销本地适配器会话失败：${error.message}`,'error');}}
  ...
  async submit(){try{
    ...
    if(!capability.connectionRequired){
      const prompt=String(inputs.prompt||inputs.text||'').trim();
      if(!prompt)throw new Error('直接生成能力需要 prompt 文本输入');
      const result=await this.world.authoring.generateAsset(prompt,{assetId,provider:capability.provider,operation:capability.operation,label:assetId,profile:this.profile.value.trim()||null});
      if(result?.status==='generator_not_configured')throw new Error(result.hint||'资产生成能力不可用');
      if(result?.status==='rejected')throw new Error(`资产准入被拒绝：${(result.admission?.reasons||[]).join(', ')||'未知原因'}`);
      this.directAsset={assetId:result.id||assetId,admission:result.admission||null,status:'asset-ready'};
      this.selectedJobId=null;
      this.compileAssetId.value=this.directAsset.assetId;
      this.costConfirm.checked=false;
      this.setState('ready','资产已就绪',`${this.directAsset.assetId} · 已通过校验与准入`);
      this.log(`资产已直接生成：${this.directAsset.assetId}`,'result');
      this.resultDisclosure.open=true;
      this.renderSelected();
      return;
    }
    if(this.world.generation.connectorStatus().status!=='paired')throw new Error('该能力来自本地适配器，请先完成配对');
    const job=await this.world.generation.submitGenerationJob({provider:capability.provider,operation:capability.operation,inputs,profile:this.profile.value.trim()||null,outputRoles:capability.output?.roles||[]});
    this.directAsset=null;
    this.assetIds.set(job.jobId,assetId);
    this.selectedJobId=job.jobId;
    this.compileAssetId.value=assetId;
    this.costConfirm.checked=false;
    this.log(`本地生成任务已提交：${job.jobId}${job.reused?' · 已复用':''}`,'result');
    await this.refresh({remote:false});
    this.jobsDisclosure.open=true;
    this.resultDisclosure.open=true;
  }catch(error){this.setState('error','提交失败',error.message);this.log(`提交生成失败：${error.message}`,'error');}finally{this.renderCapabilityHint();}}
  async refresh({remote=false,silent=false}={}){try{const result=remote?await this.world.generation.reconcileGenerationJobs():this.world.generation.listGenerationJobs();this.jobs=result.jobs||[];if(this.selectedJobId&&!this.jobs.some((job)=>job.jobId===this.selectedJobId))this.selectedJobId=null;if(!this.selectedJobId&&this.jobs.length)this.selectedJobId=this.jobs[0].jobId;this.renderJobs();this.renderSelected();if(!silent&&result.status==='connection-required'&&this.jobs.length)this.setState('connection-required','离线生成任务快照','连接器不可用；当前显示本地脱敏快照。');}catch(error){if(!silent){this.setState('error','生成任务恢复失败',error.message);this.log(`刷新生成任务失败：${error.message}`,'error');}}}
  renderJobs(){this.jobList.replaceChildren();for(const job of this.jobs){const button=el('button','generation-job-row');button.type='button';button.classList.toggle('selected',job.jobId===this.selectedJobId);const top=el('span','generation-job-row-top');top.append(el('strong',null,generationStatusLabel(job.status)),el('small',null,text(job.updatedAt)));const id=el('span','generation-job-row-id',job.jobId);const progress=job.progress?.value!=null?` · ${Math.round(job.progress.value*100)}%`:'';button.append(top,id,el('span','generation-job-row-meta',`${job.provider} · ${job.stage||job.phase||'等待中'}${progress}`));button.addEventListener('click',()=>{this.selectedJobId=job.jobId;this.resultDisclosure.open=true;this.renderJobs();this.renderSelected();});this.jobList.append(button);}if(!this.jobs.length)this.jobList.append(el('div','generation-empty','暂无本地适配器任务。服务器直接生成不会创建本地 Job。'));}
  selectedJob(){return this.jobs.find((job)=>job.jobId===this.selectedJobId)||null;}
  renderSelected(){
    const job=this.selectedJob(),outcome=job?this.outcomes.get(job.jobId):null,imported=job?this.imports.get(job.jobId):null,actions=generationJobActions(job,outcome);
    this.selectedId.textContent=job?.jobId||this.directAsset?.assetId||'未选择';
    this.jobDetail.replaceChildren();
    if(!job&&this.directAsset){
      this.jobDetail.append(el('strong',null,'服务器直接生成完成'),el('div',null,`资产 ${this.directAsset.assetId} 已注册并通过准入。`));
    } else if(!job){
      this.jobDetail.append(el('div','generation-empty','本地适配器任务会在这里显示；服务器直接生成会直接显示资产结果。'));
    } else {
      ...
      const artifacts=el('div','generation-artifacts');artifacts.append(el('strong',null,'产物'));
      for(const artifact of job.artifacts||[])artifacts.append(el('div',null,`${artifact.id} · ${artifact.role} · ${artifact.mime} · ${artifact.bytes} 字节 · ${artifact.hash}`));
      if(!(job.artifacts||[]).length)artifacts.append(el('small',null,'提供方尚未给出产物描述。'));
      this.jobDetail.append(artifacts);
    }
    const remembered=job?this.assetIds.get(job.jobId):null;
    if(job&&remembered&&!this.compileAssetId.value.trim())this.compileAssetId.value=remembered;
    this.buttons.jobRefresh.disabled=!actions.canRefresh;
    this.buttons.cancel.disabled=!actions.canCancel;
    this.buttons.import.disabled=!actions.canImport;
    this.buttons.compile.disabled=!actions.canCompile;
    this.buttons.spawn.disabled=!(actions.canSpawn||Boolean(this.directAsset&&!job));
    this.renderResult(imported,outcome,this.directAsset&&!job?this.directAsset:null);
  }
  renderResult(imported,outcome,directAsset=null){this.resultReport.replaceChildren();if(directAsset){this.resultReport.append(el('strong',null,'资产就绪'),el('div',null,`资产 ${directAsset.assetId} 可直接加入世界。`));return;}if(imported){this.resultReport.append(el('strong',null,`产物 ${imported.artifact.integrity}`),el('div',null,`${imported.artifact.id} · ${imported.artifact.mime} · ${imported.artifact.hash}`));const lineage=imported.artifact.lineage?.parents||[];this.resultReport.append(el('small',null,`上游血缘：${lineage.length?lineage.join(', '):'无'} · 生产任务：${text(imported.artifact.producer?.jobId)}`));}if(outcome){const admission=outcome.admission||{};this.resultReport.append(el('strong',null,`${outcomeStatusLabel(outcome.status)} · 准入 ${admissionStatusLabel(admission.status)}`),el('div',null,`资产 ${text(outcome.assetId)} · 原因 ${(admission.reasons||[]).join(', ')||'无'}`));if(outcome.status==='asset-rejected')this.resultReport.append(el('div','generation-error','提供方已成功，但编译器 / 准入检查拒绝该资产；不会提升为可用资产。'));}if(!imported&&!outcome)this.resultReport.append(el('div',null,'生成完成不代表可以直接进入世界；必须先通过编译与准入检查，才能加入世界。'));}
  ...
  async spawnSelected(){
    ...
    const outcome=job?this.outcomes.get(job.jobId):null;
    const assetId=this.directAsset&&!job?this.directAsset.assetId:outcome?.assetId;
    if(!assetId)return;
    if(job&&!generationJobActions(job,outcome).canSpawn)return;
    try{const result=await this.tools.call('spawnAsset',{assetId,position:[1.5,0,1.2]});this.log(`生成资产已加入世界：${assetId} · ${typeof result==='string'?'就绪':'编辑器暂定'}`,'result');}
    catch(error){this.log(`加入生成资产失败：${error.message}`,'error');}
  }
}

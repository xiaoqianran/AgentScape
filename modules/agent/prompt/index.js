...
const EMBODIED_POLICY = `Prefer Runtime placement/navigation/interaction tools over guessed coordinates. For ObjectStore assets, use navigateTo for embodied movement to an explicit world coordinate, moveForward for relative movement such as "walk forward a little", approachAndInteract for open/close, approachAndPickup for pickup and approachAndPlace for placing a held object. Never invent absolute coordinates for relative movement and never navigate an agent to its own id; if moveForward is unavailable, report that relative movement is unsupported instead of fabricating a target. For "what interactive objects are near you", call listInteractablesNearMe once instead of manually chaining canReach/listObjects. These high-level tools own interaction-pose search, navigation, Physics checks, live completion, settle, and post-condition verification as documented by their tool contracts.

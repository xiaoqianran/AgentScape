import { applyWorldRevisionProposal, buildWorldRevisionContext, classifyWorldRevisionImpact } from '../spec/WorldRevision.js';
...
const currentResolvedAssets=(runtime,worldIR)=>{
  if(!runtime.store?.get || typeof runtime.assetModule?.getManifest!=='function') return null;
  const resolved=[];
  for(const entity of worldIR.entities){
    ...
    try { runtime.assetModule.getManifest(record.assetId); } catch { return null; }
...
const canIncrementPosition=(runtime,baseWorldIR,nextIR,impact)=>{
  ...
  if(runtime.currentWorldRevision?.revision?.id!==baseWorldIR.revision?.id) return false;
  if(!runtime.store?.get || typeof runtime.assetModule?.getManifest!=='function' || !runtime.physics?.checkManifestPose || !runtime.physics?.setPosition || !runtime.validator?.run) return false;
  ...
  let manifest;
  try { manifest=runtime.assetModule.getManifest(record.assetId); } catch { return false; }
...
const occupiedWorldPositions=(runtime,worldIR,excludeId)=>{
  ...
  for(const entity of worldIR.entities){
    ...
    let record,manifest;
    try { record=runtime.store.get(entity.id); manifest=runtime.assetModule.getManifest(record.assetId); } catch { return null; }
...
const verifyIncrementalRevision=(runtime,{compilation,nextIR,revisionId,source,behaviorAdmission=null,physicsAdmission=null,layoutAdmission=null,timelineName,started})=>{
  ...
  const admission=evaluateWorldAdmission({validation,acceptance,behaviorAdmission,physicsAdmission,layoutAdmission});
...
async function recompileInitialState(runtime,{nextIR,compilation,before,previous,baseRevisionId,revisionId,impact}){
  ...
  try{
    ...
    if(admission.status==='rejected'){
      ...
      restoreWorldAuthority(runtime,previous);
      return {
    ...
    commitWorldAuthority(runtime,{
      worldIR:nextIR,
      behaviorBundle:compilation.behaviorBundle,
      physicsRequirements:compilation.physicsRequirements,
      acceptanceBundle:pipeline?.state?.artifacts?.acceptanceEvidence || null
    });
    return {
    ...
    try { await runtime.restore(before); restoreWorldAuthority(runtime,previous); }
    catch(rollbackError){
...
async function recompileAuthorityOnly(runtime,{nextIR,compilation,baseRevisionId,revisionId,impact}){
  ...
  const admissionResult=isBehavior
    ? admitWorldBehavior(compilation.behaviorBundle,{resolvedAssets,getManifest:(assetId)=>runtime.assetModule.getManifest(assetId)})
    : admitWorldPhysics(compilation.physicsRequirements,{profile:runtime.physics?.profile?.()||null,resolvedAssets,getManifest:(assetId)=>runtime.assetModule.getManifest(assetId)});
  ...
  if(admissionResult.status==='rejected'){
    const admission=evaluateWorldAdmission(admissionContext);
    return {
...
async function recompilePosition(runtime,{nextIR,compilation,before,previous,baseRevisionId,revisionId,impact}){
  ...
  let record,manifest;
  try { record=runtime.store.get(id); manifest=runtime.assetModule.getManifest(record.assetId); }
  catch { return null; }
  ...
  if(layoutAdmission.status==='rejected'){
    const admission=evaluateWorldAdmission({layoutAdmission});
    return {
...
export async function recompileWorldRevision(runtime,{baseWorldIR,proposal,acceptChangedPlan=false,pipeline=null}={}){
  ...
  const previous=captureWorldAuthority(runtime);

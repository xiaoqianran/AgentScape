...
export function registerCoreSkills(registry, runtime) {
  ...
  add('canReach', meta('基于静态 NavMesh 判断两个世界位置是否真实连通；与 findFreeSpace 不同，它回答可达性。', ['spatial.read'], ['start', 'end'], { start: vec3, end: vec3, maxSnapDistance: { type: 'number', minimum: 0 } }), (a) => runtime.navigation.canReach(a.start, a.end, { maxSnapDistance: a.maxSnapDistance }));
  add('findPath', meta('基于静态 NavMesh 计算两个世界位置之间的 Detour 路径、路径长度与端点吸附信息。', ['spatial.read'], ['start', 'end'], { start: vec3, end: vec3, maxSnapDistance: { type: 'number', minimum: 0 } }), (a) => runtime.navigation.findPath(a.start, a.end, { maxSnapDistance: a.maxSnapDistance }));

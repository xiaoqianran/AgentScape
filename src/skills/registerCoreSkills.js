...
export function registerCoreSkills(registry, runtime) {
  ...
  add('open', { ...meta('低层 articulation motor request：只请求 open target；不要把返回当成关节已完成。具身 Agent 应使用 approachAndInteract 获得 live completion。', ['world.write'], ['id'], { id: string, partName: string }), mutates: true }, (a) => runtime.interactions.setArticulationAction(a.id, 'open', { partName: a.partName }));
  add('close', { ...meta('低层 articulation motor request：只请求 close target；不要把返回当成关节已完成。具身 Agent 应使用 approachAndInteract 获得 live completion。', ['world.write'], ['id'], { id: string, partName: string }), mutates: true }, (a) => runtime.interactions.setArticulationAction(a.id, 'close', { partName: a.partName }));
  ...
  add('getArticulationStatus', meta('读取 articulated object 的 live joint 状态：当前 coordinate、requestedAction、verifiedAction，以及 moving/completed/failed/unverified observer 结果。不会把 motor request 当成完成。', ['world.read','physics.read'], ['id'], { id:string, partName:string }), (a) => runtime.interactions.articulationStatus(a.id,a.partName));

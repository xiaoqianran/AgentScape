...
export function registerCoreSkills(registry, runtime) {
  ...
  add('navigateTo', { ...meta('让可导航 Agent Body 沿当前 Detour 路径真实行走；Rapier CharacterController 负责碰撞/台阶，直到 arrived 或 blocked 才返回。', ['world.write', 'spatial.read', 'physics.read'], ['id', 'end'], { id:string, end:vec3, speed:{type:'number',exclusiveMinimum:0,maximum:8} }), mutates:true }, (a) => runtime.locomotion.navigate(a.id, a.end, { speed:a.speed }));

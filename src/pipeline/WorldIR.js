...
const ACCEPTANCE_KEYS=new Set(['id','targetId','capability','description']);
...
export function normalizeWorldIR(input={}){
  ...
  const acceptance=(input.acceptance||[]).map((item,index)=>{const label=`WorldIR acceptance[${index}]`; assertObject(item,label); assertKnownKeys(item,ACCEPTANCE_KEYS,label); const id=uniqueId(acceptanceIds,clean(item.id),label),descriptionText=clean(item.description); if(!descriptionText&&!clean(item.targetId)&&!clean(item.capability)) throw new TypeError(`${label} requires description, targetId, or capability`); return {id,...(clean(item.targetId)?{targetId:clean(item.targetId)}:{}),...(clean(item.capability)?{capability:clean(item.capability).toUpperCase()}:{}),...(descriptionText?{description:descriptionText}:{})};});

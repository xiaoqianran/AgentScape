...
Prefer place/findFreeSpace over guessing coordinates. If findPath is blocked, use suggestNavigationActions for provisional diagnosis; never treat its counterfactual recommendation as world truth. Execute any suggested interaction explicitly, then call findPath again after the world state changes. Move embodied agent objects with navigateTo instead of moveObject; navigateTo returns only after the physical walk arrives or becomes blocked. For embodied open/close tasks, call approachAndInteract directly; it already performs interaction-pose search, navigateTo, distance/physical line-of-sight checks, and action-sweep clearance. Do not manually decompose a normal embodied open/close goal into findInteractionPose + navigateTo + open unless approachAndInteract fails and you are diagnosing why. Its interaction-requested result confirms the motor request, not that articulation has fully settled; after that result, report exactly that distinction and do not loop trying to prove settled state unless a dedicated observation tool exists. For embodied pickup, call approachAndPickup instead of low-level pickup; held means kinematic-anchor ownership and explicitly does not mean grasp force verification. Use dropHeld to release an Agent-held object. Never claim a world mutation succeeded unless the tool result confirms it.

export const navigationGapScenario = {
  ...
  title: "Gap / Doorway",
  subtitle: "墙体留通行门洞",
  description: "在连续地面中央放两段墙，中间留 1.4m 通道，验证 agent-radius 侵蚀后仍能穿过门洞。",
  async setup(ctx) {
  ...
  assertions(ctx) {
    ...
    return [
      ...
      { label: "门洞允许导航通过", pass: debug.route?.reachable === true, detail: debug.route?.reason || `cost=${debug.route?.cost}` },
      { label: "直线路径穿过中央门洞区域", pass: crossesCenterGap, detail: `${path.length} waypoints` }
    ];
  }
};

export const collisionScenario = {
  ...
  title: "Collision",
  subtitle: "落体碰撞平台",
  description: "动态刚体落到固定平台，观察 native collider 与求解结果。",
  inspect: "impact-box",
  setup(ctx) {
  ...
  assertions(ctx, clock) {
    ...
    return [
      ...
      clock.frame < 150 ? { label: "最终进入低速/休眠", status: "pending" } : { label: "最终进入低速/休眠", pass: motion?.sleeping || (motion?.linearSpeed ?? 99) < 0.08, detail: `speed=${(motion?.linearSpeed ?? NaN).toFixed(4)}` }
    ];
  }
};

...
it('upgrades fallback collision and physics when a heavy provider is configured', async () => {
  const provider = { isConfigured:()=>true, endpoint:'x', run:async()=>({ collision:{strategy:'coacd',colliders:[{shape:'convexHull',vertices:[0,0,0,1,0,0,0,1,0,0,0,1]}]}, physics:{mass:3,friction:.7}, geometry:{watertight:false,windingConsistent:true,components:2} }) };

...
describe('cross-provider support-plane E2E',()=>{
  it('converges Modal 2D, Modal 3D and EmbodiedGen on one Connector snapshot and one registry',async()=>{
    ...
    expect(registry.getProviderSource('modal-2d')).toMatchObject({kind:'placeholder'});
    expect(registry.getProviderSource('modal-3d')).toMatchObject({kind:'placeholder'});
    expect(registry.getProviderSource('embodiedgen')).toMatchObject({kind:'placeholder'});
  });
});

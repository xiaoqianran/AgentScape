...
describe('WorldPhysicsAdmission',()=>{
  it('compiles body class into backend-neutral capabilities and admits matching articulated assets',()=>{
    ...
    const result=admitWorldPhysics(bundle,{backend:backend(),resolvedAssets:[{id:'door',assetId:'cabinet'}],getManifest:()=>({parts:{door:{joint:{type:'revolute'}}}})});
  ...
  it('fails closed when the authoritative backend lacks a required capability',()=>{
    ...
    expect(admitWorldPhysics(bundle,{backend:backend(),resolvedAssets:[{id:'door',assetId:'cloth'}],getManifest:()=>({})})).toMatchObject({status:'rejected',issues:[{code:'PHYSICS_BACKEND_CAPABILITY_MISSING',entityId:'door',capability:'soft-body'}]});
  });
  it('fails closed when declared quality or articulated evidence is missing',()=>{
    ...
    const result=admitWorldPhysics(bundle,{backend:b,resolvedAssets:[{id:'door',assetId:'cabinet'}],getManifest:()=>({parts:{}})});

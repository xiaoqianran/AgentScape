...
describe('WorldIR',()=>{
  ...
  it('keeps INSIDE as canonical spatial input instead of only a derived Runtime relation',()=>{
    const ir=normalizeWorldIR({schema:WORLD_IR_SCHEMA,schemaVersion:1,revision:{id:'inside-rev'},provenance:{source:'planner'},intent:{name:'Storage'},entities:[{id:'cup_01',asset:{assetId:'cup'}},{id:'cabinet_01',asset:{assetId:'cabinet'}}],spatial:{relations:[{subject:'cup_01',predicate:'inside',object:'cabinet_01',receptacleId:'interior'}]}});
    expect(ir.spatial.relations).toEqual([{subject:'cup_01',predicate:'INSIDE',object:'cabinet_01',receptacleId:'interior'}]);
  });

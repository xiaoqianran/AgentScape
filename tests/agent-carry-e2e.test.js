...
async function setup({agent=[0,0,3],cup=[0,0,0],wall=null}={}){
...
async function drive(promise,ctx,max=1200){
  let done=false,result,error; promise.then(v=>{done=true;result=v},e=>{done=true;error=e});
  for(let i=0;i<120&&!done&&ctx.locomotion.tasks.size===0;i++) await new Promise(r=>setTimeout(r,0));
  ...
  if(error) throw error; if(!done) throw new Error('task did not settle'); return result;
}
...
describe('agent carry ownership',()=>{
  it('approaches, transfers a Cup to the Agent hold anchor, carries it, and restores Dynamic body on drop',async()=>{
    ...
    const dropped=ctx.interactions.dropHeld('agent_01');
    expect(dropped).toMatchObject({status:'dropped',targetId:'cup_01'});

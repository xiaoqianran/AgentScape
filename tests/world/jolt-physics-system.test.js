...
describe('PhysicsSystem + JoltPhysicsBackend',()=>{
  it('attaches semantic manifests, simulates, syncs pose and serves scene queries',async()=>{
    ...
    try {
      ...
      physics.addObject('floor',{physics:{body:'fixed',colliders:[{shape:'box',halfExtents:[4,.5,4]}]}},floorObject);
      ...
      physics.addObject('box',{physics:{body:'dynamic',mass:2,friction:.5,colliders:[{shape:'box',halfExtents:[.5,.5,.5]}]}},boxObject);
      ...
      const obstacles=physics.getNavigationObstacles();

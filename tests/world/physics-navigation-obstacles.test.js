...
describe('PhysicsSystem navigation obstacle snapshots',()=>{
  it('reports dynamic roots and articulated Parts from current Rapier collider poses',async()=>{
    ...
    physics.addObject('cab',cabinetManifest,root);

    const before=physics.getNavigationObstacles();
    ...
    const opened=physics.getNavigationObstacles().items[0];
  ...
  it('uses conservative collider-derived AABBs for tilted cylinders and convex hulls',async()=>{
    ...
    physics.addObject('x',manifest,root);
    const snapshot=physics.getNavigationObstacles();
  ...
  it('projects upright and tilted capsules conservatively instead of skipping them',async()=>{
    ...
    physics.addObject('upright',capsuleManifest,uprightRoot);
    const upright=physics.getNavigationObstacles().items.find((item)=>item.objectId==='upright');
    ...
    physics.addObject('tilted',capsuleManifest,tiltedRoot);
    const snapshot=physics.getNavigationObstacles();

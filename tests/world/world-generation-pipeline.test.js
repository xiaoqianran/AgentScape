...
const runtimeAssetModule=(registry,catalog=new AssetCatalog({registry}))=>({
  getManifest:(assetId)=>registry.getManifest(assetId),
  hasAsset:(assetId)=>registry.has(assetId),
  catalog
});
...
describe('generated world pipeline',()=>{
  ...
  it('keeps Runtime canonical pipeline strict while the legacy entry remains an explicit compatibility boundary',async()=>{
    ...
    const compatible=await createWorldPipeline({
      ...
      assetModule:{hasAsset:()=>false,catalog:{resolveExisting:(query)=>({status:'missing',query,assets:[]})}},
  ...
  it('admits an asset published by the Generation boundary, spawns it, validates the world, and preserves provisional admission',async()=>{
    ...
    const generatedManifest={
      ...
      physics:{body:'fixed',colliders:[{shape:'box',halfExtents:[1,.5,.4],translation:[0,.5,0]}]},
      provenance:{admission:{status:'provisional',reasons:['UNVERIFIED_PROVIDER_SEMANTICS','COMPILER_UNVERIFIED']}}
    };
    const generation={
    ...
    const runtime={
      events:null,trace:null,assetModule:runtimeAssetModule(assets,assetCatalog),generation,
    ...
    const result=await createWorldPipeline(runtime).run({
      name:'Generated Lab',generation:{provider:'snapshot-provider',generate:true},
      assets:[{id:'bench_01',prompt:'industrial robotics workbench',position:[2,0,-1]}]
    });
    expect(generation.resolveAssetRequest).toHaveBeenCalledWith(expect.objectContaining({query:'industrial robotics workbench',provider:'snapshot-provider',generate:true,instanceId:'bench_01'}));
    expect(assets.has('generated-workbench')).toBe(true);
    expect(runtime.spawn).toHaveBeenCalledWith('generated-workbench',{position:[2,.01,-1],id:'bench_01'});
    expect(result.state.artifacts.worldSpec).toMatchObject({schema:1,name:'Generated Lab',generation:{provider:'snapshot-provider',generate:true}});
    ...
    expect(result.state.reports.assetAdmission).toEqual({
      ...
      provisional:[{assetId:'generated-workbench',reasons:['UNVERIFIED_PROVIDER_SEMANTICS','COMPILER_UNVERIFIED']}]
    });
    expect(result.state.reports.worldAdmission).toMatchObject({status:'provisional',reasons:['ASSET_PROVISIONAL'],validation:{hard:0,advisory:0},assets:{status:'provisional'}});
  ...
  it('marks a world rejected when a required generated asset cannot be resolved',async()=>{
    ...
    const runtime={
      events:null,trace:null,assetModule:runtimeAssetModule(assets,assetCatalog),generation,spawn:vi.fn(),
      environment:{layout:{bounds:{min:[-5,-5],max:[5,5]},groundY:0,margin:.5}},physics:{checkManifestPose:vi.fn(()=>({checked:true,clear:true,blockedBy:[]}))},
      interactions:{place:vi.fn(),move:vi.fn()},sceneGraph:{changed:vi.fn(),update:vi.fn()},
      validator:{run:vi.fn(()=>structuredClone(validation))},repair:{repair:vi.fn()},serialize:vi.fn(()=>({})),store:{get:vi.fn()}
    };
    const result=await createWorldPipeline(runtime).run({generation:{provider:'snapshot-provider',generate:true},assets:[
  ...
  it('auto-composes a missing asset position before spawning and records deterministic layout evidence',async()=>{
    ...
    const spawned=[];
    const runtime={
      events:null,trace:null,assetModule:runtimeAssetModule(assets),
...
it('executes canonical INSIDE relation through a verified receptacle placement instead of inventing a scene-graph edge',async()=>{
  ...
  const runtime={events:null,trace:null,assetModule:runtimeAssetModule(assets),environment:{layout:{bounds:{min:[-4,-4],max:[4,4]},groundY:0,margin:.5}},physics:{checkManifestPose:vi.fn(()=>({checked:true,clear:true,blockedBy:[]}))},spawn:vi.fn(async(_assetId,{id})=>id),interactions:{place:vi.fn(),placeInside,move:vi.fn()},sceneGraph:{changed:vi.fn(),update:vi.fn()},validator:{run:vi.fn(()=>structuredClone(validation))},repair:{repair:vi.fn()},serialize:vi.fn(()=>({schema:'agentscape.scene'})),store:{get:vi.fn()}};

...
const behaviorRuntime=({revisionId='behavior-rev-1',actions=['pickup','place','move'],assetId='crate'}={})=>{
  ...
  const runtime={
    ...
    snapshot:vi.fn(()=>({scene:'before'})),restore:vi.fn(async()=>{}),clearObjects:vi.fn(async()=>{}),
    pipeline:{run:vi.fn(async()=>({state:{reports:{worldAdmission:{status:'ready',reasons:[]}}},timeline:[]}))},
    store:{get:vi.fn(()=>record)},assetModule:{getManifest:vi.fn(()=>({id:'crate',actions}))},
...
const physicsRuntime=({revisionId='physics-rev-1',capabilities=['rigid-body','collision'],deterministic=true}={})=>{
  ...
  const runtime={
    ...
    snapshot:vi.fn(()=>({scene:'before'})),restore:vi.fn(async()=>{}),clearObjects:vi.fn(async()=>{}),
    pipeline:{run:vi.fn(async()=>({state:{reports:{worldAdmission:{status:'ready',reasons:[]}}},timeline:[]}))},
    store:{get:vi.fn(()=>record)},assetModule:{getManifest:vi.fn(()=>({id:'crate',actions:['move'],physics:{body:'dynamic'}}))},
    physics:{
      backend,
      profile:()=>({
...
const positionRuntime=({revisionId='position-rev-1',boxPosition=[0,.01,0],poseClear=()=>({checked:true,clear:true,blockedBy:[]}),validation={ok:true,counts:{hard:0,advisory:0},findings:[]}}={})=>{
  ...
  const runtime={
    ...
    snapshot:vi.fn(()=>({scene:'before'})),restore:vi.fn(async()=>{}),clearObjects:vi.fn(async()=>{}),
    pipeline:{run:vi.fn(async()=>({state:{reports:{worldAdmission:{status:'ready',reasons:[]}}},timeline:[]}))},
    store:{get:vi.fn((id)=>records[id])},assetModule:{getManifest:vi.fn((id)=>positionManifest(id))},interactions,
    physics:{checkManifestPose:vi.fn(poseClear),setPosition:vi.fn()},invalidateNavigationForObject:vi.fn(),environment:{layout:{bounds:{min:[-5,-5],max:[5,5]},groundY:0,margin:.5}},
...
it('incrementally moves one relation-free entity only after shared layout/Physics preflight',async()=>{
  ...
  expect(rt.invalidateNavigationForObject).toHaveBeenCalledWith('box','world.revision.position');

...
function fakeRuntime() {
  ...
  return {
    version: '0.8.0', store,
    assetModule: { getManifest: vi.fn(() => ({ id: 'cabinet', type: 'cabinet', source: { kind: 'glb', url: 'assets/cabinet.glb' }, actions: ['open'] })) },
...
it('persists environment identity and rejects cross-world restore before mutation', async () => {
  ...
  const runtime={
    ...
    assetModule:{assertCompatibleManifest:vi.fn(),hasAsset:vi.fn(()=>true)},
    clearObjects:vi.fn(),
    sceneGraph:{batch:vi.fn(async(operation)=>operation())}
  };

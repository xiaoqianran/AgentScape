...
### 由这个解释直接推出的下一组候选（**尚未实测**）

既然症结是“孤立窄带被蚀光”，就让**窄带自己够宽**。判据要求
`max_r min(r·sin(halfAngle), radiusOuter − r, r − radiusInner) ≥ 0.30`。

在 `radiusOuter = 1.75` 下反解：`sin(halfAngle) ≥ 0.30 / (1.75 − 0.30) = 0.2069`
→ `halfAngle ≥ 11.94°`。而 `halfAngle = 0.46 · sweep / stairN`：

```text
sweep = 270°, stairN = 10   →  halfAngle = 12.42°, s = 0.2151
                               内切半径 = 0.310  ≥ 0.30  ✓（余量很小）
riser = 3.12 / 11 = 0.2836  ≤ 0.30  ✓（余量也变小了）
radiusOuter 取 1.85 可把内切半径余量提到 0.328
```

**所以下一轮该动的是 `stairN`（14 → 10）——这是我至今还没通过生成器试过的变量。**
前面所有扫描都在动半径，而半径已被证明到顶也只到第 7 阶。

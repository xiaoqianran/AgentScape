...
### 第二个必要条件：踏步必须与二楼楼板**在平面上重叠**

上一节只解决了“踏步带自身能不能成立”。连通还有第二个独立条件，而且它才是原设计真正的死因：

- Recast 的连通靠**同一片区域内的 4 邻接可走单元**；而腐蚀是对**合并后区域的边界**做的，
  不是对每一块分别做。
- 原设计 `rO = 1.10 < HOLE_R = 1.20`：踏步**全部落在楼板开口之内**，与楼板在平面上毫无重叠。
  楼板可走区从 `HOLE_R + walkableRadius = 1.20 + 0.30 = 1.50` 起，
  踏步带可走区止于 `rO − walkableRadius = 1.10 − 0.30 = 0.80`。
  **中间 0.70 m 空档，且两者不属于同一区域 → 永远连不上**，无论踏步曲率多好。
- 底部能连上，是因为最底阶 `yTop = 0.208 ≤ maxClimb 0.30`，与一楼地面高差在爬升之内 → 合并成一片。
  顶部没有对应关系，所以顶部永远是孤岛。**这正是 `stair-top -> 2F` 只能靠 0.42 m 吸附的原因。**
- 所以必须 **`rO > HOLE_R`**，让踏步外缘伸到楼板之下，先在平面上重叠、合并成一片区域，再一起腐蚀。

两个条件必须**同时**满足：

```text
(a) 带自身可走：  max_r min( r·sin(halfAngle),  rEff − r,  r − rI ) ≥ 0.30     其中 rEff = min(rO, HOLE_R)
(b) 与楼板连通：  rO > HOLE_R
(c) 爬升：        riser = 3.12 / (N + 1) ≤ 0.30        →  N ≥ 10
(d) 平台净空：    落在扇形平台扇区（30°–150°）内的踏步，其 yTop ≤ FLOOR_TOP − 1.70 = 1.42
(e) 起始角可自由选（`thetaEnd`），应让平台扇区覆盖**最低**的几阶，以满足 (d)
```

一组满足 (a)–(c) 的候选（**尚未实测，仅按判据推算**）：

```text
HOLE_R 1.20 → 1.55        rO 1.10 → 1.70      （rO > HOLE_R，外缘探入 0.15）
N 14 → 11                 扫掠 270° → 360°     treadHalf 0.46·dTheta → 0.52·dTheta
rI 0.14 不变              riser = 3.12/12 = 0.26 ≤ 0.30
rEff = min(1.70, 1.55) = 1.55, halfAngle = 17.018°, s = 0.2927
→ (a) 0.2927 · 1.55 / 1.2927 = 0.351 ≥ 0.30 ✓
RAIL_R 1.24 → 1.75（必须在 HOLE_R 之外）
```

装饰环、扇形平台、`avoidR` 都引用 `HOLE_R` 符号，所以生成器里**只需改 `HOLE_R` 与 `RAIL_R` 两个字面量**。
源 HTML 的精确锚点已核对（`index.html` 第 1062 / 1166 / 1274 / 1276 行）：

```text
1062 |             const HOLE_R = 1.2, FLOOR_TOP = 3.12;
1166 |             const RAIL_R = 1.24, RAIL_H = 0.85, D2R = Math.PI / 180;
1274 |             const STAIR_N = 14;
1276 |                 const g = new THREE.Group(); const N = STAIR_N; ... const dTheta = 270 / N; const rI = 0.14, rO = 1.1; ... const treadHalf = dTheta * 0.46; const thick = 0.06;
```

**但这一组我没有实测，所以不写进生成器。** 理由：它还会牵动楼梯下的储物箱、`platformBoxes`、
`CabinFurnitureCollision` 与墙面距离，这些还没量过；把未验证的几何提交进生成器，
等于制造一个“跑一次才知道对不对”的错误楼层。
下一步应先用同样办法在探针里把这一组跑通（需同时在内存里放大楼板开口）。
...
2. 组合组（`HOLE_R` 1.5 + `rO` 1.75）确实第一次让中段出现 navmesh，并让
   ...
   回归来源**已在下面的九组扫描里定位**：是 `HOLE_R > 1.2` 与 `rO > HOLE_R` 同时成立所致，
   与楼梯下储物箱、家具代理或墙面**无关**（那个猜测是错的）。
3. 因此**未提交任何几何改动**。只提交了 `must_replace` 守卫（`165945d`），
   并已重新生成把 `cabinContents.js` 恢复为与 HEAD 逐字节一致。

### 九组扫描（一次跑完，九项指标同时盯）

写了一个临时驱动：自动改写生成器 → 重新生成 → 跑探针 → 汇总。每行都是真实重新生成后的实测。
`R` = REACHABLE，`b` = blocked。

```text
candidate      midNav  tris | 1F>2F 2F>1F door 1F>bot bot>mid mid>top
baseline            0   180 | b    b    R    R    b    b
h1.50 r1.55         0   171 | b    b    b    b    b    b
h1.50 r1.60         1   171 | b    b    b    b    b    b
h1.50 r1.65         2   174 | b    b    b    b    R    b
h1.50 r1.70         2   171 | b    b    b    b    R    b
h1.50 r1.75         3   172 | b    b    b    b    R    b
h1.35 r1.45         0   169 | b    b    b    b    b    b
h1.35 r1.65         2   176 | b    b    b    b    R    b
h1.20 r1.40         0   173 | b    b    R    R    b    b
```

`midNav` = 高度 1.00–2.50 的 navmesh 三角形总数；`door` = `outside -> 1F-inside (door open)`；
`1F>bot` = `1F-inside -> stair-bottom`；`bot>mid` = `stair-bottom -> stair-mid(k7)`；
`mid>top` = `stair-mid(k7) -> stair-top(k13)`。

**三条结论：**

1. **回归条件被精确定住：当且仅当 `HOLE_R > 1.2` 且 `rO > HOLE_R`（两者同时成立）时，
   `door` 和 `1F>bot` 从 REACHABLE 变成 blocked。**
   两个单变量用例都是干净的（`h1.20 r1.40` → 都 R；只改 `HOLE_R` 到 1.5、`rO` 保持 1.1 → 都 R。
   后者是手动跑过的）。上一版把回归归因于 `rO` 太大，**这个归因是错的，已更正**。
2. **中段 navmesh 与 `bot>mid` 同时出现在 `rO ≥ 1.65`**（即探入量 ≳ 0.15 m，约一个 cell）。
   `rO` 1.60 时中段已有 1 个三角形但 `bot>mid` 仍不通；1.65 起跨 7 个踏步连通。
   与“探入量需超过一个 cell”的预期一致。
3. **`mid>top` 在全部九组里始终 blocked**，`1F>2F` 也始终 blocked。
   即：**上半段螺旋从来没连通过**，这是与“探入量”无关的另一个问题。

另外顺手查清了 `floorShape` 的身份（源 `index.html` 1154–1159）：
它是一个 8×8 方板，`rotateX(-PI/2)` 后 `translate(0, 3, 0)`，厚 0.12 → **y ∈ [3.00, 3.12]**，
带半径 `HOLE_R` 的圆孔。所以它是 **2F 楼板**，放大开口**不是**“在一楼地面上挖洞”。
1F 地面是另一块 `PlaneGeometry`（y ≈ 0）。

### 逐阶二分：断裂的**性质**确定了（本轮最重要结论）

探针新增一个阶段：把每一对相邻踏步 `tread(k) -> tread(k+1)` 单独查询（13 个查询），
直接读出第一处断点，而不是从长路径反推。

```text
baseline    有 navmesh 的阶 {0,1} 与 {12,13}    断 11 处：PARTIAL_PATH 0，OFF_NAVMESH 11
h1.50 r1.65 有 navmesh 的阶 {0..7} 与 {12,13}   断  5 处：PARTIAL_PATH 0，OFF_NAVMESH 5
h1.50 r1.75 同 h1.50 r1.65
h1.35 r1.65 同 h1.50 r1.65
```

**所有断裂 100% 是 `OFF_NAVMESH`**——不是“有网格但连不上”，而是**那一阶干脆没有 navmesh**。
这把问题性质彻底定死了。

**有 navmesh 的阶恰好是两端**，而两端恰好是能合并进大块平面的位置：

```text
  step 13 y=2.912 ─┐
  step 12 y=2.704 ─┴─ 与 2F 楼板高差 ≤ maxClimb 0.30 → 合并进楼板     ✅ 有 navmesh
        ⋮
  step  8..11         与楼板、地面都相距 > 0.30 m → 孤立窄带
  step  2..7          → 被 walkableRadius 0.30 从切向两侧蚀光           ❌ 无 navmesh
        ⋮
  step  1 y=0.416 ─┐
  step  0 y=0.208 ─┴─ 与 1F 地面高差 0.208 ≤ 0.30 → 合并进地面        ✅ 有 navmesh
```

**最终解释（现有实测支撑，不再是推断）：**

> 一阶踏步只有当它**合并进一块大平面**时才能在 navmesh 里存活。
> 两端各有一个大平面（底 = 1F 地面，顶 = 2F 楼板），中间没有——
> 中间是一条**孤立窄带**，`walkableRadius = 0.30 m` 会从切向两侧把它蚀光。

- 净空**再次**被排除：断点处上方 4 m 内无实体（r0.35/0.6/0.9 三个采样都是 `none`）。
- 变宽只能把存活区从 2 阶延长到 8 阶（`rO ≥ 1.65`），**延不到顶**，且已到极限
  （`rO` 1.65 与 1.75 结果逐项相同）。
- `12->13` 在 baseline 就通——因为它俩合并进楼板。这解释了之前那个“顶部反而通”的怪现象。
- 这也解释了 `mid>top` 为何恒失败：7 有网格、8 没有，链在 7→8 断掉；
  而 12/13 虽有网格，与 7 之间隔着 8..11 四个空洞。

### stairN 扫描：楼梯本体**打通了**

先修正了一处判据算错：可走区的外界是`rEff = min(rO, HOLE_R)`，不是 `rO`——
超出开口的地方楼板压在头上，低净空会屏掉那些单元。上一版用 `rO` 报出的 0.248/0.317 **偏高**。
改正后的反解：`rEff ≥ agentRadius · (1 + sin(halfAngle)) / sin(halfAngle)`。

四个候选全部越过 0.30，**全部跑通**：

```text
candidate             N   h      rO    rEff  inscribed  riser  midNav   逐阶
────────────────────────────────────────────────────────────────────────────────
baseline             14  1.20   1.10   1.10   0.146 ✗   0.208     0   断 11 处
N10 h1.70 r1.90      10  1.70   1.90   1.70   0.300 ✓   0.284     7   9/9 全通
N10 h1.85 r2.05      10  1.85   2.05   1.85   0.326 ✓   0.284     9   9/9 全通
N11 h1.90 r2.10      11  1.90   2.10   1.90   0.310 ✓   0.260     7   10/10 全通
N12 h2.05 r2.25      12  2.05   2.25   2.05   0.311 ✓   0.240     8   11/11 全通
```

```text
                        bot>mid   mid>top    door    1F>bot   1F>2F
  baseline                 b         b         R        R        b
  N10 h1.70 r1.90          R         R ←首次   b        b        b
  N10 h1.85 r2.05          R         R ←首次   b        b        b
  N11 h1.90 r2.10          R         R ←首次   b        b        b
  N12 h2.05 r2.25          R         R ←首次   b        b        b
```

**三条结论：**

1. **判据是强预测器**：`inscribed < 0.30` → 楼梯几乎全断（11 处）；
   四个 `≥ 0.30` 的候选 → **整条楼梯每一对相邻踏步全部连通**。四中四，无例外。
2. **`mid>top` 首次变成 REACHABLE**，上半段螺旋打通。
3. **`stairN` 是有效杠杆**：14 → 10~12，配合开口与半径，能把楼梯**本体**完全打通。

### 剩余问题收敛成**一个**：一楼

```text
  ✅ 楼梯本体        全部 riser 连通（四个候选全覆盖）
  ✅ 上半段 mid>top   首次打通
  ❌ 一楼            door / 1F>bot 被回归打坏
  ❌ 1F <-> 2F       因为必须先走“一楼 → 楼梯底”这一跳，而它断了
```

回归条件仍是 `HOLE_R > 1.2 且 rO > HOLE_R`，四个候选全部满足。

### 下一次实验（已定，未跑）

**怀疑：放大的楼梯与一楼家具相交。** 楼梯外缘从 1.10 扩到 1.90~2.25 m，
正好伸进餐桌 / 椅子 / 坩埚 / 楼梯下储物箱所在的半径带；
相交产生的非流形 trimesh 会在**地板层**打出洞，而 1F 路径恰好走在那里。

判法：同几何下跑一次“把家具整体标为 `navigationIgnore`”的 A/B——
若 `door` 恢复 R，则原因确认；若不变，则与家具无关，再查几何本身。

### 附：探针已改为**自配置**

探针原先把 `STAIR_N 14 / 270° / r.10` 硬编码在内部。
一旦生成器改成 N=10，采样点就全部打偏——而它仍会报出看起来很确定的数字。
现已改为**从生成产物 `cabinContents.js` 正则回读**八个常数（`STAIR_N` / 扫掠 / `rI` / `rO` /
`treadHalf` 系数 / `HOLE_R` / `RAIL_R` / `FLOOR_TOP`），读不到则回退并打印警告。
重构后重跑基线：`tris 180`、分布、`broken 11`、逐阶图**逐项一致**。
另：采样点改为随实际阶数走（`MID_STEP` / `UPPER_STEP` / `TOP_STEP`），不再写死 k7/k10/k13。

### 一楼回归：家具被排除，真因是一楼地板被切薄 + **旧采样点落进边缘带**

**家具 A/B 否证了家具假设。** 把 150 个交互对象共 829 个网格全部标为 `navigationIgnore`
（其中 61 个原本真的进了 navmesh）后重烘：

```text
N10 h1.70 r1.90   door b  PARTIAL_PATH wp=4  finalDistance=1.772   ← 排除家具前也是 1.772
N12 h2.05 r2.25   door b  PARTIAL_PATH wp=4  finalDistance=1.499   ← 排除家具前也是 1.499
```

路径长度、停住位置**逐位相同** → 与家具无关。
（顺带确认：`navMeshes 286 / ignoredMeshes 1454`，家具里确实**有 61 个网格在 navmesh 内**，
但它们在三个候选里数量完全不变。）

**真因从末端路径点看得很清楚：**

```text
baseline  door 通：…[-0.8,0.09,-0.35] [-0.8,0.39,-1.25] [-0.2,0.09,-1.55] [2,0.09,-1.85]
                              ↑ 贴着地板走，只在一处短暂踩上楼梯

N10 候选  door 断：[0,0.09,6] [0.4,0.09,3.55] [0.85,0.29,1] [1.3,1.19,-0.65] ← 停在此
                                                          ↑ 沿楼梯爬到 y=1.19 就没了
```

两层原因：

1. **采样点失效（探针问题）**：`1F-inside = [2,0,-2]`，r = 2.83。
   楼梯放大后，可走区的径向范围为 `[rI + 0.30, rEff − 0.30]`，而**不可走阴影**延伸到约 `rEff + 0.30`；
   地板可走环从约 `rEff + 0.30` 才开始。`rEff = 2.05` 时地板环从 2.35 起，
   而该点吸附到 `[2.05,0.09,-1.85]`（r = 2.76）——**落在一个孤立碎片上**，
   所以 `1F>bot` 只有 wp=2 就停。这个点是当初为原始几何（rO=1.10）选的，**不随几何自适应**。
2. **真实效应**：不可走阴影从 1.40 扩到 2.35~2.55，一楼可用面积明显变小（地板三角形 122 → 101），
   而路由器会改选“穿过楼梯”的近路，爬到半途断掉。

### 陈旧采样点已被排除（已实测）

上一版把“一楼回归有多少是采样点造成的”列为关键待查项。**已查完：不是采样点。**

做法不是换一个点，而是换成**一整环 36 个点**（9 个角度 × 4 个半径，半径为 `HOLE_R + {0.6,0.9,1.3,1.7}`，
随几何推导，涵盖旧点 r=2.83），逐点记录**吸附距离**——吸附太远就说明该点根本没落在 navmesh 上，
那种 “blocked” 不能读成地板坏了。

```text
baseline 烘焙     36 个候选：落在网格上（startSnap ≤ 0.10）10 个，往返可达 0 个
重建后烘焙        36 个候选：落在网格上 9 个，往返可达 0 个
最小 startSnap   0.09   ← 与系统自身对明显在网格上的点所用的吸附量级一致
```

**所有落在网格上的候选都给出 `PARTIAL_PATH`，没有一个是 `START_OFF_NAVMESH`。**
一楼侧是健康的：路径起点确实在地板上，然后**在半途（楼梯）断掉**。
所以“陈旧采样点掩盖了成功”这个可能**被排除**——换成 36 个点，结论一字不变。

### 本轮新增的两个探针能力

1. **在屋内重建楼梯时，现在会报告实际被采集的几何**（`collectStaticMeshes()` 的网格数与类型分布），
   并**把同一套重建几何单独烘一次**。这两条把“房屋在阻断”与“踏步在阻断”分开。
2. `PROBE_INSITU_SPEC` 环境变量可切换在屋内的候选几何，不必改文件就能多组对比。

### 决定性对比：同一套几何，单独烘 9 个三角形，屋里 0 个

用合成扫描里唯一成功的那组参数（`N11 360° overlap .52 rI .08 rO 1.40`）走**屋内的生产构造**：

```text
                    单独烘焙                屋内烘焙
  三角形数          9（y 0.667 → 2.767）   0（mid 高度 1.00~2.50 全为 0）
  lowest>middle     REACHABLE (snap .392)   BLOCK
  middle>highest    PARTIAL_PATH            BLOCK
```

```text
  rO = 1.40 但 HOLE_R = 1.20
  → 可走区外界 rEff = min(rO, HOLE_R) = 1.20
  → 判据按楼板压住后重算：0.27 < 0.30
```

**抹掉那条带的不是房屋整体，而是楼板开口本身。** 这条以前是解析推断，现在是直接对比测量。

### 判据必须降级：它是必要条件，不是充分条件

`inscribed ≥ 0.30` 不能再当设计规则用。反例：

```text
  N10 360° overlap .70 rI .14 rO 1.10   （不放大开口，仅改角宽）
  rEff = 1.10
  inscribed = 0.328 ≥ 0.30  ✓  meetsAgentRadius = true
  riser = 0.284 ≤ 0.30      ✓
  → 单独烘焙：NAVMESH_EMPTY（meshCount 22，skipped 空），0 个三角形
  → 屋内烘焙：mid 高度无一有网格，1F↔2F 双向 BLOCK
```

它满足必要条件却**烘不出任何东西**。与那四个成功候选相比，差别不在内切半径（0.328 反而更大），
而在**腐蚀后径向带的宽度**：

```text
  N10 overlap.70 rO1.10    rEff 1.10        → 带 [0.44, 0.80]              宽 0.36 m   烘出 0
  四个成功候选             rEff 1.70~2.25   → 带 [rI+0.30, rEff−0.30]      宽 ≈ 0.9~1.4 m  烘出 7~9
```

所以判据要补一条：**除内切半径 ≥ 0.30 外，腐蚀后的径向带还必须足够宽**
（当前数据支持 ≥ 约 0.9 m）。这条是归纳出来的，只有两组正样本，**标记为待验证**。

### 下一次实验（已定，未跑）

本轮**没有**在真实小屋里测那四个成功候选——因为屋内重建目前**不会同时改大楼板开口**
（默认 `rO` 1.10 / `HOLE_R` 1.20），而本轮测的三组正是“只改踏步、不动开口”。

要把那四个候选真正落到真实小屋里，必须先让重建**连楼板开口一起重做**：

```text
  1. 让屋内重建在重做楼梯的同时，把二楼平台按新 h 重新开洞（当前开口是生成产物里的固定几何）
  2. 用 N10 h1.70 r1.90 跑屋内 1F↔2F
  3. 同时报告地板可走面积（baseline 122 个地板三角形 → 新开口下是多少），
     确认放大开口不会把一楼/二楼自己的平面打坏
```

在它跑通之前，阶段 2 的状态是：**归因完整、判据降级为必要条件、成功候选只在合成条件下成立。**

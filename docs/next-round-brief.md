...
上半段（`mid>top`）是现在唯一的、与探入量无关的阻塞项，先把火力集中到它：
把 `k7..k13` 逐个作为端点做二分，定位断在第几阶，再看那一阶上方是什么实体
（重点是扇形平台 [30°,150°] 与楼板底面的净空，以及上段扶手柱是否穿出楼板）。
在 `mid>top` 被解开之前，不要再调半径——半径已经证明只能打开下段，且会带回归。

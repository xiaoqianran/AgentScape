...
把一楼的采样点改成**随几何自适应**：半径取 `rEff + 0.8`（必定落在地板环内），
并在楼梯开口那 90° 扇区内取角（不落在踏步上方）。然后重测：

```text
  outside -> 1F-inside-outer      验证“一楼回归”里有多少是采样点造成的
  1F-inside-outer -> 2F           若此跳通了，整条 1F↔2F 可能直接成立
```

这一步很关键：**如果只是采样点的问题，那四个候选可能已经真的把 1F↔2F 打通了**，
只是被一个陈旧的查询点掩盖了。在改任何几何之前必须先排除这个可能。

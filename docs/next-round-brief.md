...
- **原因未定**，但已知不是：栏杆、踏步高、净空、以及当前参数空间内任何单点调参。
- 当前最强的待证解释（**明确标记：尚未确认，不得当结论使用**）：
  单个踏步在**切向**只有 `0.92 × dTheta` 的角宽，在 `r = 1.0` 处弧长约 0.31 m、在 `r = 0.6` 处约 0.19 m，
  而 `walkableRadius` 要从两侧各腐蚀 0.30 m；叠加边缘 span 的 ledge 过滤后，
  踏步作为“径向 0.96 m × 切向 0.19–0.34 m”的窄条**基本被蚀光**，
  只有在与一楼地板或二楼楼板合并的位置才幸存。这个解释**不能解释** `agentRadius 0.15`（仅 1 格腐蚀）时仍未连通，
  所以它只是候选，不是结论。
- **下一步决定性实验（尚未执行）**：把楼梯子树单独烘一次。
  能出连续带 → 是房屋周边几何在阻断；同样只出碎三角 → 是踏步自身几何。
  在该实验完成前，**不得**对楼梯几何下定论，也不得把阶段 2 标为“已知原因”。

...
本地文件在 `File.arrayBuffer()` 前先检查 `file.size`；URL 输入先检查 `Content-Length`，未知长度则流式读取并在超过 100 MiB 时立即取消。Compiler 内部仍保留同一上限作为第二道防线。预算常量只维护在 `modules/asset/production/compiler/resourceBudget.js`。

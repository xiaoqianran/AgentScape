...
export class ResourceLibrary {
  constructor({ root, resources, placement, openEnvironment = null, openGeneratedWorld = null, log = () => {} } = {}) {
    if (!root || !resources?.snapshot || !resources?.localArtifact) throw new TypeError('ResourceLibrary requires Studio root and StudioResources');
  ...
  init() {
    ...
    this.unsubscribers.push(this.resources.onChange(() => this.render()));
    this.render();
    return this;
  }
  ...
  imageCard(image) {
    ...
    const local = this.resources.localArtifact(image.id);
    if (local?.data) {
      const url = URL.createObjectURL(new Blob([local.data],{type:image.mime}));

...
export function createAppShell({ app, environmentDefinition, environments }) {
  ...
  return {
    ...
    getLatestBuildOutput:(kind)=>useStudioStore.getState().buildOutputs.find((output)=>output.kind===kind) || null,
    setSelectedObjectId:(id)=>useStudioStore.getState().setSelectedObjectId(id),
